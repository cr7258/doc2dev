...
#### 3.2 业务服务实现
- [x] 实现文档处理服务 (DocumentService)
- [x] ~~实现向量操作服务~~ (已集成到 DocumentService 中)
- [ ] ~~实现元数据操作服务~~ (简化为工具函数或集成到现有服务)
- [ ] ~~实现查询服务~~ (DocumentService 已提供完整查询功能)

**说明**: 经过实际实现发现，DocumentService 已经提供了完整的文档生命周期管理，包括加载、分割、嵌入、存储和搜索功能。额外的专门服务会增加不必要的复杂性，违背了简洁设计原则。

#### 3.3 数据模型定义
- [x] 仓库数据模型 (Repository ORM 模型已完成，包含 RepositoryStatus 枚举)
- ~~[ ] 文档数据模型~~ (删除 - DocumentService 直接使用向量存储，无需额外文档表)
- ~~[ ] 文档块数据模型~~ (删除 - LangChain 向量存储已处理块级管理)
- ~~[ ] 处理任务数据模型~~ (删除 - 当前项目不需要复杂的异步任务跟踪)
- [x] RepositoryService 集成 (已完成 - 提供完整的仓库 CRUD 操作)

**当前状况分析**:
- ✅ **仓库级元数据**: 已有完整的 Repository ORM 模型和 RepositoryService
- ✅ **向量存储**: DocumentService 直接管理，通过 LangChain 处理
- ✅ **简化架构**: 避免过度设计，保持系统简洁高效

**架构决策说明**:
1. **简化设计**: 移除不必要的中间层，DocumentService 直接操作向量存储
2. **状态管理**: Repository.repo_status 字段已足够跟踪处理状态
3. **向量关联**: 通过向量存储的 metadata 字段关联仓库信息
4. **增量处理**: 可通过 Repository.updated_at 和文件修改时间判断
