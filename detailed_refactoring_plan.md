...
#### 3.3 数据模型定义
- [x] 仓库数据模型 (已存在 repositories 表和 repository_db.py)
- [ ] 文档数据模型 (documents 表 - 跟踪文档处理状态)
- [ ] 文档块数据模型 (document_chunks 表 - 跟踪向量化的文档块)
- [ ] 处理任务数据模型 (processing_tasks 表 - 跟踪处理任务状态)
- [ ] 集成 DocumentService 与元数据管理

**当前状况分析**:
- ✅ **仓库级元数据**: 已有完整的 repositories 表和操作函数
- ❌ **文档级元数据**: 缺少文档处理状态跟踪
- ❌ **向量关联**: 向量存储与元数据库缺少关联
- ❌ **增量处理**: 无法识别新增/修改的文档

**需要实现的元数据管理**:
1. **文档跟踪**: 记录每个文档的处理状态、版本、哈希值
2. **块级跟踪**: 记录文档分割后的块与向量存储的对应关系
3. **状态管理**: 支持增量更新和错误恢复
4. **关联查询**: 元数据与向量数据的双向查询
